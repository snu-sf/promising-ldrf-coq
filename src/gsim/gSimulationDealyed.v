...
Definition is_undeferrable (e: ThreadEvent.t): bool :=
  match e with
  | ThreadEvent.syscall _ | ThreadEvent.failure => true
  | ThreadEvent.fence _ ord | ThreadEvent.write _ _ _ _ _ ord | ThreadEvent.update _ _ _ _ _ _ _ _ ord => Ordering.le Ordering.strong_relaxed ord
  | _ => false
  end.
...
Section SimulationThread.
  ...
  Definition _sim_thread_step
    ...
    forall pf_tgt e_tgt st2_tgt lc2_tgt sc2_tgt mem2_tgt st3_tgt lc3_tgt sc3_tgt mem3_tgt
           (STEPS_TGT: rtc (tau lower_step)
                           (Thread.mk _ st1_tgt lc1_tgt sc1_tgt mem1_tgt)
                           (Thread.mk _ st2_tgt lc2_tgt sc2_tgt mem2_tgt))
           (STEP_TGT: Thread.step pf_tgt e_tgt
                                  (Thread.mk _ st2_tgt lc2_tgt sc2_tgt mem2_tgt)
                                  (Thread.mk _ st3_tgt lc3_tgt sc3_tgt mem3_tgt))
           (RELEASE: is_undeferrable e_tgt)
      ...
      exists e_src st2_src lc2_src sc2_src mem2_src st3_src lc3_src sc3_src mem3_src st4_src lc4_src sc4_src mem4_src w3,
        ...
        (<<SIM: sim_thread w3 st4_src lc4_src sc4_src mem4_src st3_tgt lc3_tgt sc3_tgt mem3_tgt>>) /\
