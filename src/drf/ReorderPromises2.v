...
Lemma reorder_pf_steps_cancel
      lang
      e2 th0 th1 th2
      (STEPS1: rtc (tau (@pred_step (promise_free /1\ (fun e => ~ is_cancel e)) lang))
                   th0 th1)
      (STEP2: pred_step is_cancel e2 th1 th2)
      (LOCAL: Local.wf th0.(Thread.local) th0.(Thread.memory))
      (MEMORY: Memory.closed th0.(Thread.memory))
  ...
  :
    exists e2' th1',
      (<<STEPS1: pred_step is_cancel e2' th0 th1'>>) /\
      (<<STEPS2: rtc (tau (@pred_step (promise_free /1\ (fun e => ~ is_cancel e)) lang)) th1' th2>>)
.
Proof.
  ginduction STEPS1; i.
  - esplits; eauto.
  - inv H. dup TSTEP. inv TSTEP. inv STEP. exploit Thread.step_future.
    { eapply STEP0. }
    { eauto. }
    { eauto. }
    { eauto. } i. des.
    ...
    exploit reorder_pf_step_cancel.
    { eapply TSTEP0. }
    { eapply STEPS0. }
    { eauto. }
    { eauto. }
...
Lemma reorder_pf_steps_cancels
      lang
      th0 th1 th2
      (STEPS1: rtc (tau (@pred_step (promise_free /1\ (fun e => ~ is_cancel e)) lang))
                   th0 th1)
      (STEPS2: rtc (tau (@pred_step is_cancel lang)) th1 th2)
      (LOCAL: Local.wf th0.(Thread.local) th0.(Thread.memory))
      (MEMORY: Memory.closed th0.(Thread.memory))
      (SC: Memory.closed_timemap (Thread.sc th0) (Thread.memory th0))
  :
    exists th1',
      (<<STEPS1: rtc (tau (@pred_step is_cancel lang)) th0 th1'>>) /\
      (<<STEPS2: rtc (tau (@pred_step (promise_free /1\ (fun e => ~ is_cancel e)) lang)) th1' th2>>)
.
Proof.
  ginduction STEPS2; i.
  ...
  - exploit Thread.rtc_tau_step_future.
    ...
    exploit reorder_pf_steps_cancel.
    { eapply STEPS1. }
    { eapply TSTEP0. }
    { eauto. }
    { eauto. }
    { eauto. }
    i. des.
    dup STEPS0. inv STEPS0. inv STEP. exploit Thread.step_future.
    { eapply STEP1. }
    { eauto. }
    { eauto. }
    { eauto. } i. des.
    ...
    esplits.
    + econs 2.
      * econs; eauto. unfold is_cancel in SAT0. des_ifs.
      * eapply STEPS0.
    + eauto.
Qed.
