...
(* TODO: better place *)
Section FLATDOMAIN.
  Definition flat_le (A: Type): option A -> option A -> Prop :=
    fun a0 a1 =>
      match a0, a1 with
      | _, None => True
  ...
  Global Program Instance flat_le_PreOrder A: PreOrder (@flat_le A).
  Next Obligation.
  Proof.
    ii. destruct x; ss.
  Qed.
...
Module SeqCell.
  Record t :=
   ...
   Definition write (v: Const.t): t := mk v None.

   Definition init (v: Const.t): t := mk v (Some v).

   Definition release (c: t): t := mk c.(cur) (Some c.(cur)).

   Definition update (v: Const.t) (c: t): t :=
     match c.(rel) with
     | Some _ => mk v (Some v)
     | None => mk v None
     end.
   ...
   Definition le (c0 c1: t): Prop :=
     c0.(cur) = c1.(cur) /\ flat_le c0.(rel) c1.(rel).

   (* checked every moment *)
   Definition le_partial (c0 c1: t): Prop :=
     flat_le c0.(rel) c1.(rel).
   ...
   Proof.
     ...
     unfold le in *. ss. des. splits; etrans; eauto.
   Qed.

   Program Instance le_partial_PreOrder: PreOrder le_partial.
   Next Obligation.
   Proof.
     ii. destruct x; ss. unfold le_partial in *. ss. refl.
   Qed.
   Next Obligation.
   Proof.
     ii. destruct x, y, z; ss.
     unfold le_partial in *. ss. etrans; eauto.
   Qed.
   ...
   Lemma le_le_partial c0 c1 (LE: le c0 c1)
     :
       le_partial c0 c1.
   Proof.
     destruct c0, c1. unfold le, le_partial in *. ss. des; auto.
   Qed.
End SeqCell.
...
Section LANG.
  ...
  | na_local_step_read
      ...
      (VAL: Perm.le Perm.full (p loc) -> (m loc).(SeqCell.cur) = val)
    :
      na_local_step
        p
...
Definition update_mem
  ...
  fun loc =>
    ...
    match (d loc) with
    | diff_none => (m0 loc)
    | diff_rel | diff_relval => SeqCell.release (m0 loc)
