...
Module Flags.
  ...
  Definition init: t := fun loc => Flag.normal.

  Definition update (loc: Loc.t) (f: t): t :=
    fun loc' => if Loc.eq_dec loc' loc then Flag.race else (f loc').
End Flags.
...
Module SeqMemory.
  ...
  Definition update (loc: Loc.t) (val: Const.t) (m: t): t :=
    fun loc' => if Loc.eq_dec loc' loc then val else (m loc').
End SeqMemory.
...
Section LANG.
  ...
  Variant na_local_step (p: Perms.t):
    forall (e: MachineEvent.t)
           (pe: ProgramEvent.t)
           (m0: SeqMemory.t) (f0: Flags.t)
           (m1: SeqMemory.t) (f1: Flags.t), Prop :=
  ...
  | na_local_step_read
      ...
      (VAL: Perm.le Perm.readable (p loc) -> m loc = val)
    :
      na_local_step
        p
  ...
  | na_local_step_write
      ...
      (PERM: e = if Perm.le Perm.readable (p loc) then MachineEvent.silent else MachineEvent.failure)
      (FLAG: f1 = if Perm.le Perm.writable (p loc) then f0 else Flags.update loc f0)
    :
      na_local_step
        p
  ...
  | na_step_intro
      st0 st1 m0 m1 f0 f1 e pe
      (LANG: lang.(Language.step) pe st0 st1)
      (LOCAL: na_local_step p e pe m0 f0 m1 f1)
    :
      na_step p e (mk st0 m0 f0) (mk st1 m1 f1)
  .
...
Definition match_mem (k: kept) (mem_src mem_tgt: SeqMemory.t) :=
...
Section LANG.
  ...
  Definition certified (mem: SeqMemory.t) (th0: t): Prop :=
    exists th1,
      (<<STEPS: rtc (step MachineEvent.silent) th0 th1>>) /\
      (<<MEM: SeqMemory.flag_le th1.(state).(SeqState.memory) mem>>).

  Lemma certified_same mem th
        (MEM: th.(state).(SeqState.memory) = mem)
    :
      certified mem th.
  Proof.
    exists th. splits.
