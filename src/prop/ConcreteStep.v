...
Lemma concrete_elt_promise_step
      ...
      (WF1': Local.wf lc1 mem1')
      (CONCRETE_ELT1: Memory.concrete_elt mem1 mem1'):
  exists mem2',
    <<STEP': Local.promise_step lc1 mem1' loc from to msg lc2 mem2' kind>> /\
    <<CONCRETE_ELT2: Memory.concrete_elt mem2 mem2'>>.
Proof.
  inv WF1. inv WF1'. inv STEP.
  exploit Memory.concrete_elt_promise_exists; try exact PROMISE; eauto.
  i. des.
  esplits; eauto. econs; eauto.
  eapply Memory.concrete_elt_closed_message; eauto.
Qed.

Lemma concrete_elt_read_step
  ...
  <<STEP': Local.read_step lc1 mem1' loc ts val released ord lc2>>.
Proof.
  inv CONCRETE_ELT1. inv STEP.
  exploit SOUND; eauto. i. des; ss.
  esplits; eauto.
Qed.

Lemma concrete_elt_write_step
...
Proof.
  ...
  exploit Memory.concrete_elt_promise_exists; try exact PROMISE; eauto. i. des.
  esplits; eauto.
Qed.

Lemma concrete_elt_program_step
...
Proof.
  inv STEP.
  - esplits; eauto.
  - exploit concrete_elt_read_step; eauto.
  - exploit concrete_elt_write_step; eauto. i. des.
    esplits; eauto.
  - exploit Memory.concrete_elt_closed; eauto. i.
    exploit concrete_elt_read_step; eauto. i. des.
    exploit Local.read_step_future; try exact LOCAL1; eauto. i. des.
    exploit Local.read_step_future; try exact x0; eauto. i. des.
    exploit concrete_elt_write_step; try exact LOCAL2; eauto. i. des.
    esplits; eauto.
  - esplits; eauto.
  - esplits; eauto.
Qed.

Lemma concrete_elt_thread_promise_step
      lang pf e e1 e2
      mem1'
      (CLOSED1: Memory.closed e1.(Thread.memory))
      (WF1: Local.wf e1.(Thread.local) e1.(Thread.memory))
      (WF1': Local.wf e1.(Thread.local) mem1')
      (CONCRETE_ELT1: Memory.concrete_elt e1.(Thread.memory) mem1')
  ...
  exists mem2',
    ...
    <<CONCRETE_ELT2: Memory.concrete_elt e2.(Thread.memory) mem2'>>.
Proof.
  destruct e1. destruct e2. ss. inv STEP.
  exploit concrete_elt_promise_step; eauto. i. des.
  esplits; eauto. econs; eauto.
Qed.

Lemma concrete_elt_thread_program_step
      lang e e1 e2
      mem1'
      (CLOSED1: Memory.closed e1.(Thread.memory))
      (WF1: Local.wf e1.(Thread.local) e1.(Thread.memory))
...
Proof.
  destruct e1. destruct e2. ss. inv STEP.
  exploit concrete_elt_program_step; eauto. i. des.
  esplits; eauto. econs; eauto.
Qed.

Lemma concrete_elt_thread_step
      lang pf e e1 e2
      mem1'
      (CLOSED1: Memory.closed e1.(Thread.memory))
      (WF1: Local.wf e1.(Thread.local) e1.(Thread.memory))
...
Proof.
  inv STEP.
  - exploit concrete_elt_thread_promise_step; eauto. i. des.
    esplits; eauto. econs 1. eauto.
  - exploit concrete_elt_thread_program_step; eauto. i. des.
    esplits; eauto. econs 2. eauto.
Qed.

Lemma concrete_elt_thread_opt_step
      lang e e1 e2
      mem1'
      (CLOSED1: Memory.closed e1.(Thread.memory))
      (WF1: Local.wf e1.(Thread.local) e1.(Thread.memory))
...
Proof.
  inv STEP.
  - esplits; eauto. econs 1; eauto.
  - exploit concrete_elt_thread_step; eauto. i. des.
    esplits; eauto. econs 2; eauto.
Qed.

Lemma concrete_elt_thread_rtc_all_step
      lang e1 e2
      mem1'
      (CLOSED1: Memory.closed e1.(Thread.memory))
      (WF1: Local.wf e1.(Thread.local) e1.(Thread.memory))
  ...
  exists mem2',
    ...
    <<CONCRETE_ELT2: Memory.concrete_elt e2.(Thread.memory) mem2'>>.
Proof.
  revert mem1' CLOSED1 WF1 WF1' SC1 CONCRETE_ELT1.
  induction STEPS; i.
  - esplits; eauto.
  - inv H. inv USTEP.
    exploit concrete_elt_thread_step; eauto. i. des.
    ...
    { eapply Memory.concrete_elt_closed; eauto. }
    i. des.
    exploit IHSTEPS; try exact CONCRETE_ELT2; eauto. i. des.
    esplits; eauto. econs; eauto. econs. econs. eauto.
Qed.

Lemma concrete_elt_thread_rtc_tau_step
      lang e1 e2
      mem1'
      (CLOSED1: Memory.closed e1.(Thread.memory))
      (WF1: Local.wf e1.(Thread.local) e1.(Thread.memory))
