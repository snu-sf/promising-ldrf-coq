...
Module RelThread.
  Section RelThread.
    ...
    Inductive steps rels1: forall (rels2: RelWrites.t) (e1 e2: Thread.t lang), Prop :=
    | steps_refl
        e:
    ...
    Inductive tau_steps rels1: forall (rels2: RelWrites.t) (e1 e2: Thread.t lang), Prop :=
    | tau_steps_refl
        e:
    ...
    Inductive opt_step rels1: forall (rels2: RelWrites.t) (e: ThreadEvent.t) (e1 e2: Thread.t lang), Prop :=
    | step_none
        e:
    ...
    Proof.
      ...
      specialize (IHSTEPS (RelWrites.append L e rels1)). des.
    ...
    Lemma step_rels_wf
          rels1 rels2 e e1 e2
          (RELS1: RelWrites.wf rels1 (Local.promises (Thread.local e1)) (Thread.memory e1))
          (STEP: step rels1 rels2 e e1 e2):
      RelWrites.wf rels2 (Local.promises (Thread.local e2)) (Thread.memory e2).
    Proof.
      inv STEP. eauto using RelWrites.step_wf.
    Qed.
    ...
    Lemma step_rels_disjoint
          ...
          (RELS: RelWrites.wf rels1 promises (Thread.memory e1)):
      RelWrites.wf rels2 promises (Thread.memory e2).
    Proof.
      inv STEP. eauto using RelWrites.step_disjoint.
    Qed.
    ...
    Lemma steps_rels_disjoint
          ...
          (WF: Local.wf lc (Thread.memory e1))
          (RELS: RelWrites.wf rels1 (Local.promises lc) (Thread.memory e1)):
      RelWrites.wf rels2 (Local.promises lc) (Thread.memory e2).
    Proof.
      induction STEPS; ss.
    ...
    Proof.
      ...
      replace (RelWrites.append L e rels1) with (RelWrites.append L fe rels1) in STEPS0; cycle 1.
      { unfold RelWrites.append. destruct e; inv EVENT; ss.
    ...
    Proof.
      inv STEP. unfold RelWrites.append. des_ifs; eauto.
    Qed.
...
Module RelConfiguration.
  Section RelConfiguration.
    ...
    | step_intro
        ...
        (STEP: RelThread.opt_step L ordcr ordcw rels1 rels2 e e2 e3)
    ...
    Inductive steps rels1: forall (rels2: RelWrites.t) (c1 c2: Configuration.t), Prop :=
    | steps_refl
        c:
    ...
    (*   unfold RelWrites.append in H. des_ifs. inv H; ss. inv H0. *)
    (*   eapply EVENT; eauto. *)
    (* Qed. *)
    ...
    (*           RelWrites.wf rels1 (Local.promises lc) (Configuration.memory c1)) *)
    (*       (STEP: step e tid rels1 rels2 c1 c2) *)
    (*       (WRITE: ThreadEvent.is_writing e = Some (loc, from, to, val, released, ord)): *)
    ...
    (*   hexploit (@RelThread.steps_rels_wf lang L ordcr ordcw); *)
    ...
    (*   unfold RelWrites.append. des_ifs; eauto. *)
    (* Qed. *)
...
Module RelRARace.
  Section RelRARace.
    ...
    Lemma step_racefree
          ...
          (STEP: RelConfiguration.step L ordcr ordcw e tid rels1 rels2 c1 c2):
      racefree rels2 c2.
    Proof.
