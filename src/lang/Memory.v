...
Module Memory.
  ...
  Lemma loc_non_init_max_ts
        loc mem
        (INIT: loc_non_init loc mem):
    max_ts loc mem <> Time.bot.
  Proof.
    inv INIT. des. destruct p.
    exploit max_ts_spec; eauto. i. des.
    ii. rewrite H1 in MAX. inv MAX; inv H2.
    congr.
  Qed.
  ...
  Proof.
    ...
    - left. exploit adjacent_exists; try eapply H; eauto. i. des.
      ...
      { clear MIDDLE BACK COMPLETE x x1 GET0 H.
        inv x3. rewrite GET0 in x0. inv x0.
        exploit get_ts; try exact GET2. i. des.
        { subst. inv TS. }
    ...
    - right. inv H. do 4 (split; auto).
      ...
      hexploit loc_non_init_max_ts; eauto. i.
  ...
  Inductive cap_imm (loc: Loc.t) (to: Time.t) (view: View.t) (promises mem1 mem2: t): Prop :=
  | cap_imm_intro
      (SOUND: forall loc' from' to' msg (GET: get loc' to' mem1 = Some (from', msg)),
          get loc' to' mem2 = Some (from', msg))
    ...
    - (* to = max_ts loc mem1 *)
      ...
      { exists mem1. splits; auto. econs; i; try congr.
        ...
        - inv NON_INIT. des. destruct p.
          exploit max_ts_spec; try exact H0. i. des.
          exfalso. apply H. rewrite e in *.
      ...
      + econs; i.
        ...
        * revert GET0. erewrite add_o; eauto. condtac; try congr; i.
          ...
          exists (max_ts loc mem1). esplits; eauto.
  ...
  Proof.
    ...
    - destruct (loc_ts_eq_dec (loc0, to1) (loc, to)); ss.
      ...
      + exploit MIDDLE0; eauto; cycle 1.
        ...
        * destruct (Time.le_lt_dec to to1); ss.
          ...
          exploit get_disjoint; [exact GET|exact x0|..]. i. des.
          { subst. timetac. }
          exfalso. apply (x7 to1); econs; ss; eauto.
          { econs. auto. }
          { refl. }
        * exploit get_ts; try exact GET. i. des.
          { subst. timetac. }
    ...
    - destruct (loc_ts_eq_dec (loc0, max_ts loc0 mem1) (loc, to)); ss.
      ...
      + inv IN; try by (des; congr). guardH o.
        ...
        { inv NON_INIT. des. destruct p.
          ...
          exploit get_disjoint; [exact x|exact GET0|..]. i. des.
          { subst. rewrite x6 in *. timetac. }
          exfalso. apply (x6 (max_ts loc mem1)); econs; ss; eauto.
          - refl.
          - econs. auto. }
        rewrite -> TS in *.
        ...
        { inv LATEST. exploit SOUND; eauto. i. econs; eauto.
          ...
          - exploit COMPLETE; eauto. i. des. subst.
            ...
            { subst. inv x1. exploit SOUND; try exact GET4. i.
              ...
              exfalso.
              destruct (TimeFacts.le_lt_dec to0 to2).
              - apply (x9 to0); econs; ss. refl.
              - apply (x9 to2); econs; ss.
                + econs. ss.
                + refl. }
            exploit get_disjoint; [exact GET1|exact x0|..]. i. des; try congr.
            exfalso.
    ...
    - rename GET2 into GET3.
      ...
      exploit SOUND; eauto. i. rewrite x3 in *. inv x1.
      esplits; eauto; try by econs 2; eauto.
      { inv x. des. destruct p.
        destruct (get loc0 x1 mem1) as [[]|] eqn:GET4.
      ...
      { exploit max_ts_spec; try exact x6. i. des.
        eapply TimeFacts.antisym; eauto. clear from msg1 GET5 MAX0.
        destruct (TimeFacts.le_lt_dec (max_ts loc mem1) to); ss.
        exploit get_ts; try exact GET. i. des.
        { subst. inv x5. des. destruct p.
          exploit max_ts_spec; try exact H0. i. des.
          rewrite x1 in *. inv l. }
        exploit get_ts; try exact GET0. i. des.
        { subst. inv x5. des. destruct p.
          exploit max_ts_spec; try exact H0. i. des.
          rewrite x1 in *. inv l0. }
        exploit get_disjoint; [exact GET|exact x3|..]. i. des.
        { subst. rewrite x9 in *. timetac. }
        exfalso. apply (x9 (max_ts loc mem1)); econs; ss. refl. }
      subst. rewrite GET0 in x6. symmetry in x6. inv x6.
      ...
      { subst. inv x5. des. destruct p.
        ...
        rewrite x6 in *. congr. }
      exploit get_ts; try exact x1. i. des.
      { rewrite x8, x4 in *. timetac. }
      exploit get_disjoint; [exact GET3|exact x1|..]. i. des; try congr.
      exfalso.
      destruct (TimeFacts.le_lt_dec to0 (max_ts loc mem2)).
      + apply (x9 to0); econs; ss. refl.
      + apply (x9 (max_ts loc mem2)); econs; ss.
        * econs. ss.
        * refl.
  Qed.
  ...
  Lemma concrete_max_ts
        mem1 mem2 loc
        (CONCRETE: concrete mem1 mem2)
        (CLOSED1: closed mem1):
    max_ts loc mem1 = max_ts loc mem2.
  Proof.
    exploit concrete_closed; eauto. intro CLOSED2.
    inv CONCRETE. apply TimeFacts.antisym.
    - inv CLOSED1. specialize (INHABITED loc).
      exploit max_ts_spec; try exact INHABITED. i. des.
      exploit SOUND; eauto. i. des.
      + exploit max_ts_spec; try exact x. i. des. ss.
      + exploit max_ts_spec; try exact x1. i. des. ss.
    - inv CLOSED2. specialize (INHABITED loc).
      exploit max_ts_spec; try exact INHABITED. i. des.
      exploit COMPLETE; eauto. i. des.
      + exploit max_ts_spec; try exact x. i. des. ss.
      + exploit max_ts_spec; try exact x. i. des. ss.
  ...
  Lemma concrete_half_wf
    ...
    half_wf mem2.
  Proof.
    ii. exploit concrete_max_ts; eauto. i.
    inv CONCRETE. exploit COMPLETE; eauto.
    rewrite <- x0. i. des.
    - exploit HALF; eauto. i. des.
  ...
  Inductive concrete_latest (mem1 mem2: t): Prop :=
  | concrete_latest_intro
      (SOUND: forall loc from to msg (GET: get loc to mem1 = Some (from, msg)),
          get loc to mem2 = Some (from, msg) \/
          to = max_ts loc mem1 /\
          msg = Message.half /\
  ...
  Lemma concrete_latest_concrete
        mem1 mem2
        (CONCRETE: concrete_latest mem1 mem2):
    concrete mem1 mem2.
  Proof.
    inv CONCRETE. econs; eauto; i.
    exploit SOUND; eauto. i. des; eauto.
    right. esplits; eauto.
  Qed.

  Lemma half_wf_concrete_latest_trans
        mem1 mem2 mem3
        (CLOSED: closed mem1)
        (HALF: half_wf mem1)
        (CONCRETE12: concrete_latest mem1 mem2)
        (CONCRETE23: concrete_latest mem2 mem3):
    concrete_latest mem1 mem3.
  Proof.
    exploit concrete_latest_concrete; try exact CONCRETE12. i.
    exploit concrete_latest_concrete; try exact CONCRETE23. i.
    inv CONCRETE12. inv CONCRETE23. econs; i.
    - exploit SOUND; eauto. i. des.
      + exploit SOUND0; eauto. i. des; eauto.
        subst. right.
        exploit concrete_max_ts; try exact x0; eauto. i.
        rewrite <- x3 in *. split; eauto.
        exploit COMPLETE; try exact x4. i. des.
        { split; ss. esplits; eauto. }
        { exploit HALF; try exact GET. i. des. congr. }
  ...
  Lemma lower_latest_half_concrete_latest
        mem1 loc from val released mem2 from'
        (MSG: get loc from mem1 = Some (from', Message.full val released))
        (LOWER: lower mem1 loc from (max_ts loc mem1) Message.half (Message.full val None) mem2):
    concrete_latest mem1 mem2.
  Proof.
    exploit lower_get0; eauto. i. des.
    econs; i.
    - erewrite lower_o; eauto. condtac; ss.
  ...
  Lemma latest_half_domain
        promises mem
        (LE: le promises mem)
        (CLOSED: closed mem):
    exists dom,
      <<SOUND: forall loc from
                 (GET: get loc (max_ts loc mem) mem = Some (from, Message.half))
                 (GETP: get loc (max_ts loc mem) promises = None),
        List.In loc dom>>.
  Proof.
  ...
  Lemma concrete_exists
    ...
    exists mem2,
      <<FUTURE: future mem1 mem2>> /\
      <<CONCRETE: concrete_latest mem1 mem2>> /\
      <<NOHALF: no_half_except promises mem2>> /\
      <<LE2: le promises mem2>>.
  Proof.
    exploit latest_half_domain; eauto. i. des.
    revert mem1 LE1 CLOSED1 HALF1 SOUND.
    induction dom; i.
    { esplits; eauto.
      - econs; eauto.
      - ii. destruct (get loc (max_ts loc mem1) promises) as [[f []]|] eqn:PROMISE.
        + exploit LE1; eauto. i. congr.
        + exploit LE1; eauto. i.
          rewrite GET in x. inv x. ss.
        + exploit SOUND; eauto. ss.
    }
    rename a into loc.
    exploit (@max_ts_spec loc); try eapply CLOSED1. i. des.
    clear MAX. destruct msg.
    { eapply IHdom; eauto. i.
      destruct (Loc.eq_dec loc loc0); try congr.
      exploit SOUND; eauto. i. inv x; congr. }
    destruct (get loc (max_ts loc mem1) promises) as [[]|] eqn:GETP.
    { eapply IHdom; eauto. i.
      exploit LE1; eauto. i. rewrite GET in x. inv x.
      destruct (Loc.eq_dec loc loc0); try congr.
      exploit SOUND; eauto. i. inv x; congr. }
    exploit get_ts; try exact GET. i. des.
    { subst. rewrite x1 in *.
      inv CLOSED1. rewrite INHABITED in *. inv GET. }
    exploit HALF1; eauto. i. des.
    exploit lower_exists; try exact GET; eauto.
    { instantiate (1 := Message.full val' None). econs. ss. }
    intro LOWER. des.
    exploit lower_latest_half_concrete_latest; eauto. intro CONCRETE12.
    exploit concrete_latest_concrete; eauto. i.
    exploit (IHdom mem2); i.
    { ii. erewrite lower_o; eauto. condtac; ss; eauto.
      i. des. subst. congr. }
    { eapply concrete_closed; eauto. }
    { eapply concrete_half_wf; eauto. }
    { exploit lower_get0; eauto. i. des.
      exploit concrete_max_ts; eauto. i. rewrite <- x3 in *.
      revert GET0. erewrite lower_o; eauto. condtac; ss; i.
      guardH o. exploit SOUND; try exact GET0; eauto. i. des; ss.
      subst. unguard. des; ss. }
    des. exists mem0. splits; eauto.
    - econs 2; eauto. econs; eauto.
      econs. ss. unfold TimeMap.bot. apply Time.bot_spec.
    - eapply half_wf_concrete_latest_trans; eauto.
  Qed.
End Memory.
